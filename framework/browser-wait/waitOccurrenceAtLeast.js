...
/**
 * Wait for element occurrence to be at least a given amount.
 ...
 */
export function waitOccurrenceAtLeast(element, occurrence, ms) {
    ...
    try {
        browser.waitUntil(function () {
            ...
            try {
                ...
                if (err.toString().includes('stale element')) {
                    console.log('--*-*-* waitOccurrenceAtLeast Caught: stale Element *-*-*-- Element: ' + element);
                }
        ...
        if (err.toString().includes('timeOut')) {
            throw new Error('waitOccurrenceAtLeast Timed out at ' + waitTime + ' milliseconds.\nElement count did not meet expectation.\nElement: ' + element + '\nFound Count: ' + elCount + '\nExpected Count: ' + occurrence);
        }
        else {
            throw new Error('waitOccurrenceAtLeast ' + err)
        }
    }
}